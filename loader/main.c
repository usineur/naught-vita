...
int pthread_create_soloader(pthread_t **thread, pthread_attr_t **attr, void *(*start)(void *), void *param) {
...
int pthread_mutex_init_soloader(pthread_mutex_t **mutex, const pthread_mutexattr_t **attr) {
...
FILE *fopen_hook(char *fname, char *mode) {
	...
	if (strncmp(fname, "ux0:", 4)) {
		sprintf(real_fname, "ux0:data/naught/%s", fname);
...
int stat_hook(const char *pathname, stat64_bionic *statbuf) {
	...
	if (strncmp(pathname, "ux0:", 4)) {
		char fname[256];
		sprintf(fname, "ux0:data/naught/%s", pathname);
		dlog("stat(%s) fixed\n", fname);
		res = stat(fname, &st);
	} else {
...
android_DIR *opendir_fake(const char *dirname) {
	...
	if (strncmp(dirname, "ux0:", 4)) {
		char real_fname[256];
		sprintf(real_fname, "ux0:data/naught/%s", dirname);
...
int mkdir_hook(const char *pathname, int mode) {
	...
	if (strncmp(pathname, "ux0:", 4)) {
		char real_fname[256];
		sprintf(real_fname, "ux0:data/naught/%s", pathname);
...
int rename_hook(const char *old_filename, const char *new_filename) {
	...
	if (strncmp(old_filename, "ux0:", 4)) {
		sprintf(real_old, "ux0:data/naught/%s", old_filename);
	} else {
		strcpy(real_old, old_filename);
	}
	if (strncmp(new_filename, "ux0:", 4)) {
		sprintf(real_new, "ux0:data/naught/%s", new_filename);
	} else {
...
void *pthread_main(void *arg) {
	...
	int (* engineInitialize) (void *env) = (void *)so_symbol(&main_mod, "Java_com_blueshadowgames_naught_S3DRenderer_engineInitialize");
	int (* engineRunOneFrame) (void *env) = (void *)so_symbol(&main_mod, "Java_com_blueshadowgames_naught_S3DRenderer_engineRunOneFrame");
	int (* engineSetSystemVersion) (void *env, void *obj, char *ver) = (void *)so_symbol(&main_mod, "Java_com_blueshadowgames_naught_S3DRenderer_engineSetSystemVersion");
	int (* engineSetDirectories) (void *env, void *obj, char *cache_path, char *home_path, char *pack_path) = (void *)so_symbol(&main_mod, "Java_com_blueshadowgames_naught_S3DRenderer_engineSetDirectories");
	int (* enginePause) (void *env, void *obj, int pause) = (void *)so_symbol(&main_mod, "Java_com_blueshadowgames_naught_S3DRenderer_enginePause");
	int (* engineTouch) (void *env, void *obj, int state, float x, float y, int state2, float x2, float y2, int state3, float x3, float y3, int state4, float x4, float y4, int state5, float x5, float y5) = (void *)so_symbol(&main_mod, "Java_com_blueshadowgames_naught_S3DRenderer_engineOnTouchesChange");
	...
	#define handleKey(btn, vx, vy) \
		if (((pad.buttons & btn) == btn) && !((oldpad & btn) == btn)) { \
			...
			touch.reportNum = 1; \
			touch.report[0].x = vx * 2; \
			touch.report[0].y = vy * 2; \
			oldpad = pad.buttons; \
		}
	...
	for (;;) {
		...
		for (int j = 0; j < 6; j++) {
			if (j == 0) {
				handleKey(SCE_CTRL_LTRIGGER, 128, 512);
				handleKey(SCE_CTRL_RTRIGGER, 832, 512);
				if (touch.reportNum) {
		...
		engineOnDeviceMove(fake_env, NULL, -9.81 * motion.acceleration.x, -9.81 * motion.acceleration.y, 9.81 * motion.acceleration.z);
...
int main(int argc, char *argv[]) {
	...
	char fname[32];
	sprintf(data_path, "ux0:data/naught");
